...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('dateFilter').addEventListener('change', fetchdeathRegistrations);

    fetchdeathRegistrations(); // Initial load
});
...
const fetchdeathRegistrations = () => {
    ...
    fetch(`https://civilregistrar.lgu2.com/api/death.php?${params.toString()}`)
            ...
            data.forEach(death => {
                const row = `
                    <tr class='border-b border-x border-gray-300'>
                        ...
                        <td class='px-4 py-2 text-center border-r border-gray-300'>${death.user_name}</td>
                        <td class='px-4 py-2 text-center border-r border-gray-300'>
                            <span class='${getStatusClass(death.status)} text-xs font-medium px-2.5 py-0.5 rounded'>
                                ${death.status}
                            </span>
                        </td>
                        <td class='px-4 py-2 text-center border-r border-gray-300'>
...
document.getElementById('modalConfirmButton').addEventListener('click', () => {
    ...
    if (newStatus) {
        ...
        fetch(`https://civilregistrar.lgu2.com/api/death.php`, {
            method: 'PUT',
            headers: {
...
function updateStatus(deathId) {
    ...
    fetch(`https://civilregistrar.lgu2.com/api/death.php?id=${deathId}`)
        .then(response => response.json())
        .then(data => {
            if (data && data.status) {
