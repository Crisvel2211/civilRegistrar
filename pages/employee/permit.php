           ...
           <!-- Delete Confirmation Modal -->
            ...
            <div id='viewpermitModal' class='fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden'>
                <div class='bg-white rounded-lg shadow-lg max-w-lg w-full p-6'>
                    <!-- Modal Header -->
                    <div class='flex justify-between items-center border-b pb-3 mb-4'>
                        <h3 class='text-lg font-semibold text-gray-800'>Permit Registration Details</h3>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('dateFilter').addEventListener('change', fetchpermitRegistrations);

    fetchpermitRegistrations(); // Initial load
});
...
const fetchpermitRegistrations = () => {
    ...
    fetch(`https://civilregistrar.lgu2.com/api/permit_request_api.php?${params.toString()}`)
            ...
            data.forEach(permit => {
                const row = `
                    <tr class='border-b border-x border-gray-300'>
                        ...
                        <td class='px-4 py-2 text-center border-r border-gray-300'>${permit.user_name}</td>
                        <td class='px-4 py-2 text-center border-r border-gray-300'>
                            <span class='${getStatusClass(permit.status)} text-xs font-medium px-2.5 py-0.5 rounded'>
                                ${permit.status}
                            </span>
                        </td>
                        <td class='px-4 py-2 text-center border-r border-gray-300'>
...
document.getElementById('modalConfirmButton').addEventListener('click', () => {
    ...
    if (newStatus) {
        ...
        fetch(`https://civilregistrar.lgu2.com/api/permit_request_api.php`, {
            method: 'PUT',
            headers: {
...
function updateStatus(permitId) {
    ...
    fetch(`https://civilregistrar.lgu2.com/api/permit_request_api.php?id=${permitId}`)
        .then(response => response.json())
        .then(data => {
            if (data && data.status) {
        ...
        fetch(`https://civilregistrar.lgu2.com/api/permit_request_api.php?id=${permitId}`)
            ...
            .then(data => {
                if (data) {
                    ...
                    <p><strong>PR:</strong> ${data.reference_number}</p>
                    <p><strong>Resident Full Name:</strong> ${data.resident_name}</p>
                    <p><strong>Permit Type:</strong> ${data.permit_type}</p>
                    <p><strong>Additional Details:</strong> ${data.additional_details}</p>
                    <p><strong>Status:</strong> ${data.status}</p>
                    <p><strong>Request Date:</strong> ${data.date_of_request}</p>
                    </div>
                    `;
