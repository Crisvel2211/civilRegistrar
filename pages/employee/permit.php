...
const fetchpermitRegistrations = () => {
    ...
    fetch(`http://localhost/group69/api/permit_request_api.php?employee_id=${employeeId}&search=${encodedSearch}&status=${status}`)
...
document.getElementById('modalConfirmButton').addEventListener('click', () => {
    ...
    if (newStatus) {
        ...
        fetch(`http://localhost/group69/api/permit_request_api.php`, {
            method: 'PUT',
            headers: {
...
function updateStatus(permitId) {
    ...
    fetch(`http://localhost/group69/api/permit_request_api.php?id=${permitId}`)
        .then(response => response.json())
        .then(data => {
            if (data && data.status) {
